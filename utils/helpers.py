...
def get_status_badge(status):
    ...
    return f'<span style="background:{color};color:white;padding:4px 12px;border-radius:12px;font-size:12px;">{status}</span>'
...
def get_project_balance(project_id):
    ...
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT current_balance FROM projects WHERE id = ?", (project_id,))
    result = cursor.fetchone()
    conn.close()
    return result['current_balance'] if result else 0

def get_requisition_total(req_id):
    """Get total amount for a requisition"""
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT SUM(amount) FROM transactions WHERE requisition_id = ?", (req_id,))
    result = cursor.fetchone()
...
def get_item_name(item_id):
    """Get master item name by ID"""
    if not item_id:
        return "Unknown"
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT canonical_name FROM master_items WHERE id = ?", (item_id,))
    result = cursor.fetchone()
    conn.close()
    return result['canonical_name'] if result else "Unknown"

def get_next_ref_no(project_code):
    """Generate next reference number for a project"""
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("""
        SELECT MAX(CAST(SUBSTR(ref_no, -4) AS INTEGER)) 
        FROM requisitions 
        WHERE ref_no LIKE ?
    """, (f"{project_code}%",))
    result = cursor.fetchone()
    ...
    return f"{project_code}-{datetime.now().strftime('%Y%m')}-{next_num:04d}"

def calculate_closing_balance(opening_balance, total_amount):
    """Calculate closing balance"""
    return (opening_balance or 0) + (total_amount or 0)

def is_valid_ref_no(ref_no):
    """Validate reference number format"""
    pattern = r'^[A-Z0-9]+-\d{6}-\d{4}$'
    return bool(re.match(pattern, ref_no.upper()))